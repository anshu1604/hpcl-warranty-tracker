...
const AddMakeScreen = () => {
  ...
  const [filteredMakes, setFilteredMakes] = useState<Array<Array<string>>>([]);
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        const response = await axios.get(Google_Sheet_Creds.makeMasterData);
        const makeData = (response.data.values || []).slice(1); // remove header
        setMakes(makeData);
        setFilteredMakes(makeData);
      } catch (error) {
  ...
  const addMake = async () => {
    ...
    const newId =
      makes.length > 0 ? Math.max(...makes.map(o => parseInt(o[0]))) + 1 : 1;
    const newEntry: Array<string> = [newId.toString(), trimmed];

    const updatedMakes = [...makes, newEntry];
    setMakes(updatedMakes);
    setFilteredMakes(updatedMakes);
    setNewMake('');
  };
        ...
        <View style={styles.headerContainer}>
          ...
          <View style={styles.addContainer}>
            <TextInput
              ...
              onSubmitEditing={addMake} // press Enter to add
            />
            <TouchableOpacity style={styles.addButton} onPress={addMake}>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <View style={styles.makeItem}>
              <Text style={styles.makeText}>{item[1]}</Text>
            </View>
          )}
          contentContainerStyle={{ paddingBottom: 20 }}
          ListFooterComponent={filteredMakes.length ? <Footer /> : null}
        />
      </View>
    </ScreenWrapper>
  );
